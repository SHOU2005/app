        ...
        {TABS.map(({ href, label, Icon }) => {
          ...
          return (
            <Link key={href} href={href} className="flex flex-col items-center gap-[3px] flex-1 py-1 relative">
              <Icon style={{ width: 24, height: 24, color: on ? BLUE : 'rgba(0,0,0,0.3)', strokeWidth: on ? 2.2 : 1.7, transition: 'color 0.2s' }} />
              <span style={{ fontSize: 11, fontWeight: on ? 700 : 500, color: on ? BLUE : 'rgba(0,0,0,0.3)', transition: 'color 0.2s' }}>
                {label}
              </span>
              {on && <span className="absolute -top-2 left-1/2 -translate-x-1/2 h-0.5 rounded-full" style={{ width: 20, background: BLUE }} />}
            </Link>
          )
        })}
      </div>
