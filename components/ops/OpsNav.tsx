...
import { LayoutDashboard, Users, Briefcase, BookOpen, TrendingUp, MessageSquare, Settings } from 'lucide-react'
    ...
    <>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 220, background: S1, borderRight: `1px solid ${BD}`, display: 'none', flexDirection: 'column', zIndex: 50, padding: '20px 0' }} className="ops-sidebar">
        <div style={{ padding: '0 16px 20px', borderBottom: `1px solid ${BD}` }}>
          <p style={{ color: T1, fontWeight: 800, fontSize: 18, margin: 0 }}>⚡ Ops Portal</p>
          <p style={{ color: T2, fontSize: 12, margin: '4px 0 0' }}>Operations Dashboard</p>
        </div>
        {LINKS.map(({ href, label, Icon }) => {
          const on = pathname === href || (href !== '/ops' && pathname.startsWith(href))
          return (
            <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', margin: '2px 8px', borderRadius: 10, background: on ? `${ACC}20` : 'transparent', textDecoration: 'none' }}>
              <Icon style={{ width: 18, height: 18, color: on ? ACC : T2 }} />
              <span style={{ fontSize: 14, fontWeight: on ? 700 : 500, color: on ? T1 : T2 }}>{label}</span>
            </Link>
          )
        })}
      </nav>
          ...
          {LINKS.slice(0, 5).map(({ href, label, Icon }) => {
            ...
            return (
              <Link key={href} href={href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, textDecoration: 'none', flex: 1 }}>
                <Icon style={{ width: 22, height: 22, color: on ? ACC : T2 }} />
                <span style={{ fontSize: 10, fontWeight: on ? 700 : 400, color: on ? ACC : T2 }}>{label}</span>
              </Link>
            )
          })}
        </div>
