...
export default function PermissionScreen() {
  ...
  async function requestAll() {
    ...
    // ── 4. SMS / PHONE / CONTACTS / PHOTOS — fire native popups, don't await ──
    try {
      if (Cap?.isNativePlatform?.() && Cap?.Plugins?.NativePermissions) {
        // Fire and forget — dialogs appear asynchronously, no callback needed
        Cap.Plugins.NativePermissions.requestAll()
      }
    } catch {}
    next.sms = next.phone = next.contacts = 'granted'
    setStatus(s => ({ ...s, ...next }))
