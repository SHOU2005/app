          ...
          return (
            <div key={task.id} style={{ background: overdue ? '#F7F7F7' : '#F5F5F5', border: overdue ? '1px solid #FECDD3' : '1px solid transparent', borderRadius: 14, padding: '14px 16px', marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <p style={{ fontWeight: 700, color: overdue ? '#111111' : T1, margin: 0, fontSize: 15, flex: 1 }}>{task.title}</p>
              ...
              {task.dueDate && (
                <p style={{ fontSize: 12, color: overdue ? '#111111' : T2, margin: '0 0 10px', fontWeight: overdue ? 700 : 400 }}>
              ...
              {task.status === 'IN_PROGRESS' && (
                <button onClick={() => updateStatus(task.id, 'DONE')} style={{ width: '100%', padding: '8px', borderRadius: 10, border: 'none', background: '#111111', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                  Mark Done ✓
                </button>
              )}
            </div>
