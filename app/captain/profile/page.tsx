      ...
      <div style={{ padding: '24px 20px' }}>
        ...
        {cp?.referralCode && (
          <div style={{ background: T1, borderRadius: 16, padding: '16px 18px', marginBottom: 20 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: 1.2, margin: '0 0 8px' }}>Referral Code</p>
            ...
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', margin: '8px 0 0' }}>Share to earn ₹100 per completed booking</p>
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
          {[
            ...
            { label: 'Pending Payout',value: `₹${cp?.pendingPayout ?? 0}` },
        ...
        {editing ? (
          <div style={{ background: '#F7F7F7', borderRadius: 16, padding: '20px', marginBottom: 12 }}>
            <p style={{ fontWeight: 700, color: T1, marginBottom: 16, margin: '0 0 16px' }}>Edit Profile</p>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setEditing(false)} style={{ flex: 1, padding: '12px', borderRadius: 12, border: `1px solid ${BD}`, background: '#FFFFFF', fontWeight: 700, fontSize: 14, cursor: 'pointer', color: T2 }}>Cancel</button>
              <button onClick={save} disabled={saving} style={{ flex: 1, padding: '12px', borderRadius: 12, border: 'none', background: T1, color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>{saving ? 'Saving…' : 'Save'}</button>
            </div>
          </div>
        ) : (
