...
export default function AttendancePage() {
  ...
  async function checkIn() {
    ...
    try {
      ...
      await load()
    } catch { alert('Could not get location. Please enable GPS.') }
    setBusy(false)
  }
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ background: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: 20, padding: '24px', marginBottom: 24, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: 24 }}>
            <div>
              <p style={{ fontSize: 11, color: T2, margin: '0 0 4px' }}>Check In</p>
            ...
            <div>
              <p style={{ fontSize: 11, color: T2, margin: '0 0 4px' }}>Check Out</p>
          ...
          {!loading && (
            !today?.checkInTime ? (
              <button onClick={checkIn} disabled={busy} style={{ padding: '14px 40px', borderRadius: 14, background: BLUE, color: '#fff', fontWeight: 800, fontSize: 16, border: 'none', cursor: 'pointer', width: '100%' }}>
        ...
        {/* History */}
        <p style={{ fontSize: 13, fontWeight: 700, color: T2, marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>Recent History</p>
