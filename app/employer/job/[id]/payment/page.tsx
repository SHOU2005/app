...
export default function JobPaymentPage() {
  ...
  useEffect(() => {
    fetch(`/api/employer/jobs/${id}`).then(r => r.json()).then(d => {
      if (d.job) {
        setJob(d.job)
        setBooking(d.job.bookings?.[0] ?? null)
        if (d.job.bookings?.[0]?.paymentStatus === 'PAID') setScreen('confirmed')
      }
      setLoading(false)
    }).catch(() => setLoading(false))
  }, [id])

  // Auto-trigger payment as soon as booking is ready
  useEffect(() => {
    if (!loading && booking?.id && screen === 'summary' && !autoTriggered.current) {
  ...
  async function handlePay() {
    if (!booking?.id) {
      alert('No booking found. Please go back and post the job again.')
      return
    }
    ...
    try {
      ...
      const isRealKey = orderData.keyId &&
        !orderData.keyId.includes('REPLACE') &&
        !orderData.keyId.includes('placeholder')

      if (isRealKey && orderData.orderId) {
        ...
        if (loaded) {
          ...
          await new Promise<void>((resolve) => {
            const rzp = new (window as any).Razorpay({
              ...
              modal: {
                ondismiss: () => { setScreen('summary'); resolve() },
              },
            })
            rzp.open()
          })
      ...
      /* 3. Fallback: simulate payment flow (for testing) */
      setStatusMsg('Processing payment…')
      await new Promise(r => setTimeout(r, 600))
      setStatusMsg('Verifying with bank…')
      await new Promise(r => setTimeout(r, 600))
      setStatusMsg('Confirming transaction…')

      const mockRes  = await fetch('/api/razorpay/mock-complete', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bookingId: booking.id }),
      })
      ...
      if (!mockRes.ok) {
        alert(mockData.error || 'Payment failed. Please try again.')
        setScreen('summary')
        return
      }

      await new Promise(r => setTimeout(r, 400))
      setPayRef(mockData.paymentId || `PAY${Date.now().toString().slice(-8)}`)
      setScreen('confirmed')
      ...
      <div style={{ position: 'relative', zIndex: 20, padding: 'calc(48px + env(safe-area-inset-top)) 20px calc(32px + env(safe-area-inset-bottom))' }}>
        ...
        <div style={CARD}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 13, color: T2 }}>Verified · Background checked</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 11, color: T3 }}>Rating</div>
              <div style={{ fontSize: 15, fontWeight: 800, color: GOLD }}>4.8 ★</div>
            </div>
          </div>
        </div>
        ...
        <div style={CARD}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: `1px solid ${BD}` }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: T1 }}>₹{total - gst}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: `1px solid ${BD}` }}>
            <span style={{ fontSize: 14, color: T2 }}>GST (18%)</span>
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, fontFamily: FONT, color: T1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 3 }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={GRN} strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                <span style={{ fontSize: 13, color: GRN, fontWeight: 600 }}>Job Completed</span>
              </div>
            </div>
            <div style={{ textAlign: 'right' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: BG, borderTop: `1px solid ${BD}`, padding: '14px 16px', paddingBottom: 'calc(14px + env(safe-area-inset-bottom))' }}>
        {/* Razorpay-styled button */}
        <button onClick={handlePay} style={{
          width: '100%', padding: '0', borderRadius: 16, border: 'none', cursor: 'pointer',
          background: T1,
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '17px 0' }}>
            ...
            <svg width="20" height="20" viewBox="0 0 30 30" fill="none">
              ...
              <path d="M15 2L8 22h4l3-4z" fill="rgba(0,0,0,0.4)"/>
            </svg>
            <span style={{ fontSize: 17, fontWeight: 900, color: '#000', letterSpacing: 0.2 }}>
