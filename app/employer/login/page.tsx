...
export default function EmployerLoginPage() {
  ...
  function handleOtpChange(v: string) {
    ...
    if (clean.length === 6) setTimeout(() => document.getElementById('emp-verify-btn')?.click(), 80)
  }

  return (
    <div style={{ minHeight: '100vh', background: BG, display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center', padding: '24px 24px env(safe-area-inset-bottom)', color: T1 }}>

      <div style={{ marginBottom: 40, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ width: 72, height: 72, borderRadius: 22, background: '#FFFFFF', display: 'flex', alignItems: 'center',
          justifyContent: 'center', marginBottom: 16, boxShadow: '0 12px 40px rgba(255,255,255,0.1)' }}>
          <span style={{ fontSize: 36, fontWeight: 900, color: '#000', lineHeight: 1 }}>S</span>
        </div>
        <p style={{ fontSize: 26, fontWeight: 900, letterSpacing: -0.5 }}>Switch Employer</p>
      ...
      <div style={{ width: '100%', maxWidth: 400, background: CARD, borderRadius: 28,
        ...
        <p style={{ fontSize: 22, fontWeight: 900, marginBottom: 6 }}>
          {stage === 'phone' ? 'Welcome back' : 'Enter OTP'}
        </p>
        ...
        {stage === 'phone' ? (
          <>
            ...
            <button onClick={handleSendOtp} disabled={!phoneOk || loading}
              style={{ width: '100%', height: 56, borderRadius: 16, border: 'none',
                background: phoneOk ? T1 : '#1E1E1E', color: phoneOk ? '#000' : 'rgba(255,255,255,0.15)',
          ...
          <>
            <div style={{ borderRadius: 16, border: `1.5px solid ${otpOk ? T1 : 'rgba(255,255,255,0.1)'}`,
              background: '#161616', marginBottom: 16, overflow: 'hidden' }}>
              <input type="tel" inputMode="numeric" maxLength={6} placeholder="_ _ _ _ _ _"
                value={otp} autoFocus
                onChange={e => handleOtpChange(e.target.value)}
                style={{ width: '100%', background: 'transparent', outline: 'none', border: 'none',
                  padding: '0 20px', fontSize: 32, fontWeight: 800, color: T1, letterSpacing: 12, height: 68,
                  boxSizing: 'border-box' as const }} />
            </div>
            {error && <p style={{ fontSize: 13, color: '#EF4444', marginBottom: 12, fontWeight: 600 }}>{error}</p>}
            <button id="emp-verify-btn" onClick={handleVerify} disabled={!otpOk || loading}
              style={{ width: '100%', height: 56, borderRadius: 16, border: 'none',
                background: otpOk ? T1 : '#1E1E1E', color: otpOk ? '#000' : 'rgba(255,255,255,0.15)',
            ...
            <button onClick={() => { if (countdown > 0) return; setStage('phone'); setOtp(''); setError('') }}
              ...
              style={{ width: '100%', height: 40, background: 'none', border: 'none',
                cursor: countdown > 0 ? 'default' : 'pointer',
                color: countdown > 0 ? 'rgba(255,255,255,0.2)' : T2, fontSize: 14, fontWeight: 600 }}>
      ...
      <p style={{ marginTop: 24, fontSize: 14, color: T2 }}>
        ...
        <button onClick={() => router.push('/employer/register')}
          style={{ color: T1, fontWeight: 800, background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, textDecoration: 'underline' }}>
          Register
        </button>
      </p>
    </div>
