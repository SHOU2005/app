    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: FONT, color: T1 }}>
      ...
      <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ background: CARD, borderRadius: 24, padding: '28px 24px', border: `1px solid ${BORDER}`, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 900, color: T1, marginBottom: 8, lineHeight: '34px' }}>Invite Friends, Earn ₹100</div>
        ...
        <div style={{ background: CARD, borderRadius: 20, padding: '20px', border: `1px solid ${BORDER}` }}>
          ...
          {[
            { step: '1', title: 'Share your code', desc: 'Send SWITCH100 to your friends' },
            { step: '2', title: 'Friend signs up',  desc: 'They register and post their first job' },
            { step: '3', title: 'You earn ₹100',    desc: 'Credited to your wallet instantly' },
          ].map(s => (
        ...
        <div style={{ background: CARD, borderRadius: 20, padding: '20px', border: `1px solid ${BORDER}` }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 28, fontWeight: 900, color: T1, letterSpacing: 3, lineHeight: '36px' }}>{referCode}</span>
            <button onClick={copyCode} style={{
              padding: '9px 18px', borderRadius: 10,
              background: copied ? 'rgba(16,185,129,0.15)' : T1,
              color: copied ? '#10B981' : '#000',
              border: copied ? '1px solid rgba(16,185,129,0.3)' : 'none',
              cursor: 'pointer', fontWeight: 700, fontSize: 14, fontFamily: FONT, lineHeight: '20px',
              transition: 'all 0.2s',
            }}>{copied ? 'Copied!' : 'Copy'}</button>
          </div>

          <button onClick={() => {
            const msg = `Use my referral code ${referCode} on Switch Employer app!`
            window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, '_blank')
          }} style={{
            width: '100%', padding: '15px 0', borderRadius: 16, border: 'none', cursor: 'pointer',
            background: '#25D366', color: '#fff', fontWeight: 700, fontSize: 16, lineHeight: '24px',
        ...
        <div style={{ background: CARD, borderRadius: 16, padding: '16px 18px', border: `1px solid ${BORDER}` }}>
          ...
          {['₹100 credited after friend completes first booking', 'No limit on referrals', 'Reward valid for 90 days'].map(t => (
