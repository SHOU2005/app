      ...
      <div style={{ padding: '20px', marginLeft: 0 }} className="ops-content">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingTop: 'env(safe-area-inset-top,0px)' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[7, 30, 90].map(d => (
              <button key={d} onClick={() => setDays(d)} style={{ padding: '6px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700, border: 'none', cursor: 'pointer', background: days === d ? ACC : S1, color: days === d ? T1 : T2 }}>{d}d</button>
            ))}
          </div>
        </div>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
              {[
                ...
                { label: 'Bookings',        value: data.totalBookings,                                  color: '#818CF8' },
                { label: 'Completed',       value: data.completedBookings,                              color: '#FBBF24' },
                { label: 'Workers',         value: data.totalWorkers,                                   color: '#F87171' },
                { label: 'Active Captains', value: data.totalCaptains,                                  color: '#A78BFA' },
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 100 }}>
                {data.dailyRevenue.map((d, i) => (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
                    <div style={{ width: '100%', background: ACC, borderRadius: '3px 3px 0 0', height: `${(d.revenue / maxRevenue) * 100}%`, minHeight: d.revenue > 0 ? 4 : 0 }} title={`${d.date}: ₹${d.revenue}`} />
                  </div>
                ))}
              </div>
